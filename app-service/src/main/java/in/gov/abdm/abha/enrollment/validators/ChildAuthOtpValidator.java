...
public class ChildAuthOtpValidator implements ConstraintValidator<Otp, AuthData> {
    ...
    public boolean isValid(AuthData authData, ConstraintValidatorContext context) {
        ...
        if (authData.getAuthMethods() != null && !authData.getAuthMethods().isEmpty() && authData.getAuthMethods().get(0) != null && authData.getAuthMethods().contains(AuthMethods.OTP))
        {
        ...
        else if (authData.getAuthMethods() != null && !authData.getAuthMethods().isEmpty() && authData.getAuthMethods().get(0) != null && authData.getAuthMethods().stream().noneMatch(v -> v.equals(AuthMethods.OTP)))
        {
            return otpNullorEmpty(authData) || otpNotNullorEmpty(authData);
        }
       else
           return true;
        return false;
