...
public class OtpValueValidator implements ConstraintValidator<OtpValue, String> {
    ...
    /**
     * suppress reason
     * is rsa encrypted check is required
     */
    public boolean isValid(String otp, ConstraintValidatorContext context) {
        return (otp != null && !otp.isEmpty() && isValidInput(otp) && isRSAEncrypted(otp)
                && Pattern.compile(OTP_REGEX_PATTERN).matcher(rsaUtil.decrypt(otp)).matches());
    }
