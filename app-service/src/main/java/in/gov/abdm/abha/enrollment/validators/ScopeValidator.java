...
/**
 * Validates scope attribute from the provided list of scopes against the
 * pre-decided value ie. abha-enrol
 */
public class ScopeValidator implements ConstraintValidator<ValidScope, List<Scopes>> {

	/**
	 * Implements the validation for scope field Scope should be abha-enrol for the
	 * abha creation using aadhaar flow
	 *
	...
	public boolean isValid(List<Scopes> requestScopes, ConstraintValidatorContext context) {
		if (requestScopes == null || requestScopes.isEmpty())
			return false;
		List<Scopes> enumNames = Stream.of(Scopes.values()).filter(name -> !name.equals(Scopes.WRONG))
