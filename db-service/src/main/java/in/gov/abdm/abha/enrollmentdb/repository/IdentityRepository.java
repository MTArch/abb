...
public interface IdentityRepository extends R2dbcRepository<Identity, String> {
	...
	@Query(value = "WITH TEMP_TBL AS (SELECT acc_phr.health_id_number, acc_phr.phr_address from hid_phr_address acc_phr where acc_phr.health_id_number = :abhaNumber and acc_phr.preferred = 1  )  SELECT acc.health_id_number AS abha_number,acc_phr.phr_address AS abha_address,CASE WHEN (temp_tbl.phr_address is null) THEN (acc.health_id) :: character varying ELSE temp_tbl.phr_address END AS preferred_abha_address,acc.name,acc.kycdob AS date_of_birth,acc.gender,acc.mobile,acc.email_verified AS email,acc.type AS abha_type,NULL :: text AS parent_id,acc.status,acc.state_name AS state,acc.district_name AS district,acc.address,acc.created_date,acc.day_of_birth,acc.month_of_birth,acc.year_of_birth,acc.xmluid,'accounts' :: text AS tb_nm  FROM (accounts acc LEFT JOIN hid_phr_address acc_phr ON (((acc.health_id_number) :: text = (acc_phr.health_id_number) :: text)) LEFT JOIN TEMP_TBL temp_tbl ON (((acc.health_id_number) :: text = (temp_tbl.health_id_number) :: text)))  where acc.health_id_number = :abhaNumber AND acc.status = 'ACTIVE'  order by created_date desc  limit 1")
	...
	@Query(value = "SELECT DISTINCT acc.health_id_number AS abha_number,acc_phr.phr_address AS abha_address,CASE WHEN (acc_phr.preferred = 1) THEN acc_phr.phr_address ELSE (acc.health_id)::character varying END AS preferred_abha_address,acc.name,acc.kycdob AS date_of_birth,acc.gender,acc.mobile,acc.email_verified AS email,acc.type AS abha_type,NULL::text AS parent_id,acc.status,acc.state_name AS state,acc.district_name AS district,acc.address,acc.created_date,acc.day_of_birth,acc.month_of_birth,acc.year_of_birth,'accounts'::text AS tb_nm    FROM (accounts acc LEFT JOIN hid_phr_address acc_phr ON (((acc.health_id_number)::text = (acc_phr.health_id_number)::text))) where acc.mobile = :mobileNumber AND acc.status='ACTIVE'")
