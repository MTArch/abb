...
public interface SyncAcknowledgmentRepository extends R2dbcRepository<SyncAcknowledgement, BigInteger> {
    ...
    Mono<SyncAcknowledgement> updatePatientSyncAcknowledgment(@Param("isSyncedWithPatient") Boolean isSyncedWithPatient, @Param("requestId") String requestId, @Param("healthIdNumber") String healthIdNumber);
    @Query(value = PHR_SYNC_ACKNOWLEDGEMENT_UPDATE_QUERY)
    Mono<SyncAcknowledgement> updatePhrSyncAcknowledgment(@Param("isSyncedWithPhr") Boolean isSyncedWithPhr, @Param("requestId") String requestId, @Param("healthIdNumber") String healthIdNumber);
}
